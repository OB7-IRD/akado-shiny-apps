...
A default database is created when the secure connection is activated for the first time. The database includes a user `shiny` with the password `azerty`, user type `logbook` and an administrator `shinymanager` with the password `12345`, user type `all`. These credentials must be changed immediately ! Which can be done directly within the application by logging in as the administrator.
```{yml}
