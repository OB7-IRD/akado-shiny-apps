...
FROM 
	...
	FROM (
		...
		FROM 
			ps_common.trip trip
			LEFT JOIN  ps_common.trip trip_previous_end_tide_tmp ON trip.vessel = trip_previous_end_tide_tmp.vessel AND trip_previous_end_tide_tmp.enddate <= trip.startdate AND coalesce(trip_previous_end_tide_tmp.logbookprogram,'NULL') IN (?select_item_1) AND trip_previous_end_tide_tmp.landingwellcontentstatus = (SELECT w.topiaid FROM ps_logbook.wellcontentstatus w WHERE w.code = '1') AND trip_previous_end_tide_tmp.topiaid <> trip.topiaid
			LEFT JOIN  ps_common.trip trip_current_end_tide_tmp ON trip.vessel = trip_current_end_tide_tmp.vessel AND trip_current_end_tide_tmp.enddate >= trip.enddate AND trip_current_end_tide_tmp.enddate >= trip.startdate AND coalesce(trip_current_end_tide_tmp.logbookprogram,'NULL') IN (?select_item_1) AND trip_current_end_tide_tmp.landingwellcontentstatus = (SELECT w.topiaid FROM ps_logbook.wellcontentstatus w WHERE w.code = '1')
		WHERE 
