...
FROM (
	...
	FROM 
		ps_common.trip trip
		LEFT JOIN  ps_common.trip trip_previous_tmp ON trip.vessel = trip_previous_tmp.vessel AND trip_previous_tmp.enddate < trip.startdate AND trip_previous_tmp.logbookprogram IS NOT NULL
	WHERE 
		trip.topiaid IN (?select_item)
...
WHERE 
	trip_previous.logbookprogram IS NOT NULL OR trip_previous.topiaid IS NULL 
